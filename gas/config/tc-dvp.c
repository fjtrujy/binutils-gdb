  ...
  if (last_label_seen)
      ...
      if (last_label_seen2)
	{
	  symbolS * cur_mpgloc = compute_mpgloc (mpgloc_sym, vif_data_start,
						 expr_build_dot ());
      symbol_set_value_expression(last_label_seen2, symbol_get_value_expression(cur_mpgloc));
	}
    }
...
#endif
      symbol_set_value_expression(sym, symbol_get_value_expression(cur_mpgloc));
  ...
  for (p = ovlysym_table; p; p = p->next)
    {
	 ...
	 not be a point in setting it to non-zero anyway.  */
      symbol_set_bfdsym(p->sym, symbol_get_bfdsym(p->sym) - bfd_section_vma(p->sec));
    }
}

     ...
     long stretch;
{
  ...
  /* XXX The above segfault as symbol_get_frag(symbolP) is only a valid pointer when
     ...
     symbolS *addr;
{
  ...
  if (symbol_constant_p(symbol_get_value_expression(addr)))
     ...
     symbolS *start_label, *end_label;
{
     ...
     they serve different purposes.  */
  {
    ...
    gdb_start_sym = create_colon_label (STO_DVP_VU, VUOVERLAY_START_PREFIX,
					section_name);
    symbol_set_value_expression(gdb_start_sym, symbol_get_value_expression(addr));
  }
#endif
     ...
     int internal_p;
{
  ...
  symbol_set_frag (vif_data_end, frag_now);
